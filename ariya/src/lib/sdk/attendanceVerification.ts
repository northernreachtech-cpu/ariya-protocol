...
export class AttendanceVerificationSDK {
  ...
  /**
   * Check if user has Completion NFT for a specific event
   */
  async hasCompletionNFT(
    userAddress: string,
    eventId: string,
    nftRegistryId: string
  ): Promise<boolean> {
    try {
      ...
      const tx = new Transaction();
      tx.moveCall({
        target: `${this.packageId}::nft_minting::has_completion_nft`,
        arguments: [
          tx.pure.address(userAddress),
          tx.pure.id(eventId),
          tx.object(nftRegistryId),
      ...
      if (result && result.results && result.results.length > 0) {
        ...
        if (Array.isArray(returnVals) && returnVals.length > 0) {
          const hasNFT = Array.isArray(returnVals[0])
            ...
            : returnVals[0];
          console.log(
            "🔍 Contract check - Has Completion NFT:",
            Boolean(hasNFT)
          );
          return Boolean(hasNFT);
        }
      ...
      const { data: objects } = await suiClient.getOwnedObjects({
        owner: userAddress,
        filter: {
          StructType: `${this.packageId}::nft_minting::NFTOfCompletion`,
        },
        options: { showContent: true },
      });

      console.log("📦 Found Completion NFT objects:", objects.length);
      ...
      const hasEventNFT = objects.some((obj) => {
        ...
        if (
          ...
          const fields = (content as any).fields;
          console.log("🔍 Checking Completion NFT fields:", fields);
          const isMatch = fields && fields.event_id === eventId;
          console.log("✅ Event ID match:", {
            nftEventId: fields?.event_id,
            currentEventId: eventId,
      ...
      console.log("🎯 Final Completion NFT ownership result:", hasEventNFT);
      return hasEventNFT;
    } catch (error) {
