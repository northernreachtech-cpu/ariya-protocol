    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        ...
        stars.push(
          <Star
            key={i}
            className="h-4 w-4 fill-yellow-500/50 text-yellow-500 dark:fill-yellow-400/50 dark:text-yellow-400"
          />
        );
      } else {
        stars.push(
          <Star key={i} className="h-4 w-4 text-gray-300 dark:text-gray-400" />
        );
      }
    }
    return stars;
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          ...
          <Card className="p-4 sm:p-6 mb-6 lg:mb-8">
            <div className="flex flex-col gap-4">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-foreground-muted" />
                <input
                  type="text"
                  placeholder="Search organizers..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-10 pr-4 py-3 sm:py-4 bg-card border border-border rounded-lg text-foreground placeholder-foreground-muted focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all"
                />
              </div>
            </div>
          </Card>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
            {filteredOrganizers.map((organizer) => (
              <Card
                key={organizer.id}
                className="group overflow-hidden border border-border hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/10 transition-all duration-500 transform hover:-translate-y-1"
              >
                <div className="p-4 sm:p-6 lg:p-8">
                  ...
                  <div className="grid grid-cols-1 gap-3 mb-4 sm:mb-6 sm:grid-cols-2 sm:gap-4">
                    <div className="p-3 rounded-lg bg-card-secondary border border-border">
                      <div className="text-xs text-foreground-muted font-medium mb-1">
                        Successful Events
                      </div>
                    ...
                    <div className="p-3 rounded-lg bg-green-400/10 border border-green-400/20 dark:bg-green-400/10 dark:border-green-400/20">
                      <div className="text-xs text-foreground-muted font-medium mb-1">
                        Member Since
                      </div>
                      <div className="text-green-600 dark:text-green-400 font-bold text-sm sm:text-base">
